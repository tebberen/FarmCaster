...
export const SuccessModal: React.FC<SuccessModalProps> = ({ isOpen, onClose, theme, emoji, networkName, xpEarned }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className={clsx("relative w-full max-w-sm bg-slate-900 border rounded-2xl shadow-2xl p-6 flex flex-col items-center gap-6", theme.border)}>

                <button
                    onClick={onClose}
                    className="absolute top-3 right-3 p-2 hover:bg-slate-800 rounded-full transition-colors text-slate-400 hover:text-white"
                >
                    <X size={20} />
                </button>
                ...
                <div className="text-center space-y-2 mt-4">
                    ...
                    <h2 className="text-2xl font-bold text-white">Harvest Successful!</h2>
